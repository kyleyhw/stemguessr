...
- **`POST /api/reset`** — cancels any ingest in progress (cooperative, between-track), waits for it to stop, then clears `manifest.json`, `stems/`, `previews/` from the cache. The cancel-then-join-before-delete ordering makes reset safe mid-ingest, so the button works at any time rather than being refused while ingesting. Frontend `↺ reset` chip (top-right HUD) gates it behind an "Are you sure?" confirm, shows a "Cancelling…" status during the wait, then returns to the playlist form.
